					...
					Containers: []corev1.Container{
						{
							...
							Command: []string{"korrel8r", "web", fmt.Sprintf("--https=:%d", info.Korrel8rPort), "--cert=/secrets/tls.crt", "--key=/secrets/tls.key", "--config=/config/korrel8r.yaml"},
							Ports: []corev1.ContainerPort{
								{
									ContainerPort: info.Korrel8rPort,
									Protocol:      corev1.ProtocolTCP,
								},
							},
							SecurityContext: &corev1.SecurityContext{
